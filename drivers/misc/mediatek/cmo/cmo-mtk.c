...
void inner_dcache_flush_all(void) { __inner_flush_dcache_all(); }
EXPORT_SYMBOL(inner_dcache_flush_all);

void inner_dcache_flush_L1(void) { __inner_flush_dcache_L1(); }

void inner_dcache_flush_L2(void) { __inner_flush_dcache_L2(); }

void inner_dcache_disable(void) { __disable_dcache(); }
...
void smp_inner_dcache_flush_all(void) {
  ...
  smp_call_function_many(&mask, (smp_call_func_t)inner_dcache_flush_L2, NULL,
                         true);
  /*
...
static void _smp_inner_dcache_flush_all(void) {
  ...
  for (i = 0; i < total_core; i += num_core) {
    ...
    smp_call_function_single(online_cpu, (smp_call_func_t)inner_dcache_flush_L2,
                             NULL, true);
  }
#ifdef PERF_MEASURE
  ...
  do {
    ...
    case FLUSHING:
      ...
      for (j = 0; j < npages_this_entry; j++) {
        start = (unsigned long)cache_flush_map_page_va(
            cache_flush_vm_struct[this_cpu], page++);
...
int smp_sync_cache_by_cpu_pool_ion(struct sg_table *table,
  ...
  for_each_cpu(cpu, &mask) if (cpu != this_cpu) largest_affinity_cpu = cpu;
...
int smp_sync_cache_by_cpu_pool_m4u(const void *va, const unsigned long size) {
  ...
  on_each_cpu_mask(&mask, (smp_call_func_t)_flush_range, (void *)&smp_call_args,
                   false);
