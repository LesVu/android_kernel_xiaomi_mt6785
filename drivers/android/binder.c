    ...
    int step)
{
  ...
  if ((t == NULL) || (t->log_idx == -1) || (t->log_idx > (transaction_log->size - 1)))
    return;
                                 ...
                                 struct binder_transaction_log *transaction_log_failed)
{
  ...
  transaction_log->entry =
      extmem_malloc_page_align(
          sizeof(struct binder_transaction_log_entry) * MAX_ENG_TRANS_LOG_BUFF_LEN);
  ...
  if (reply)
  {
    ...
    if (target_thread->transaction_stack != in_reply_to)
    {
      binder_user_error("%d:%d got reply transaction with bad target transaction stack %d, expected %d\n",
                        proc->pid, thread->pid,
                        target_thread->transaction_stack ? target_thread->transaction_stack->debug_id : 0,
    ...
    if (!(tr->flags & TF_ONE_WAY) && thread->transaction_stack)
    {
      ...
      if (tmp->to_thread != thread)
      {
        ...
        binder_user_error("%d:%d got new transaction with bad transaction stack, transaction %d has target %d:%d\n",
                          ...
                          tmp->to_thread ? tmp->to_thread->pid : 0);
    ...
    case BC_CLEAR_DEATH_NOTIFICATION:
    {
      ...
      if (ref == NULL)
      {
        binder_user_error("%d:%d %s invalid ref %d\n",
                          proc->pid, thread->pid,
                          cmd == BC_REQUEST_DEATH_NOTIFICATION ? "BC_REQUEST_DEATH_NOTIFICATION" : "BC_CLEAR_DEATH_NOTIFICATION",
                          target);
        binder_proc_unlock(proc);
        kfree(death);
        break;
  ...
  while (1)
  {
    ...
    case BINDER_WORK_CLEAR_DEATH_NOTIFICATION:
    {
      ...
      binder_debug(BINDER_DEBUG_DEATH_NOTIFICATION,
                   "%d:%d %s %016llx\n",
                   proc->pid, thread->pid,
                   cmd == BR_DEAD_BINDER ? "BR_DEAD_BINDER" : "BR_CLEAR_DEATH_NOTIFICATION_DONE",
                   (u64)cookie);
      if (w->type == BINDER_WORK_CLEAR_DEATH_NOTIFICATION)
      {
        binder_inner_proc_unlock(proc);
    ...
    binder_debug(BINDER_DEBUG_TRANSACTION,
                 ...
                 proc->pid, thread->pid,
                 (cmd == BR_TRANSACTION) ? "BR_TRANSACTION" : (cmd == BR_TRANSACTION_SEC_CTX) ? "BR_TRANSACTION_SEC_CTX"
                                                                                              : "BR_REPLY",
                                               ...
                                               struct binder_transaction_log_entry *e)
{
  ...
  seq_printf(m,
             ...
             e->debug_id, (e->call_type == 2) ? "reply" : ((e->call_type == 1) ? "async" : "call "),
  ...
  seq_printf(m,
             ...
             (e->call_type ==
              0)
                 ? ((unsigned long)e->endstamp.tv_sec)
                 : 0,
             (e->call_type ==
              0)
                 ? (e->endstamp.tv_nsec / NSEC_PER_USEC)
                 : 0,
...
int binder_transaction_log_show(struct seq_file *m, void *unused)
{
  ...
  count = log_cur + 1;
#ifdef BINDER_WATCHDOG
  cur = count < log->size && !log->full ? 0 : count % log->size;
