...
static int primary_display_get_round_corner_mva(unsigned int *tp_mva,
                                                unsigned int *bt_mva,
                                                unsigned int *pitch,
                                                unsigned int *height,
...
int dynamic_debug_msg_print(unsigned int mva, int w, int h, int pitch,
  ...
  if (disp_helper_get_option(DISP_OPT_SHOW_VISUAL_DEBUG_INFO)) {
    ...
    ret = MFC_Open(&mfc_handle, (void *)kva, pitch, h, bytes_per_pix,
                   DAL_COLOR_WHITE, DAL_COLOR_RED);
...
int primary_display_get_debug_state(char *stringbuf, int buf_len) {
  ...
  len += scnprintf(
      ...
      pgc->max_layer, session_mode_spy(pgc->session_mode), pgc->vsync_drop);
  len += scnprintf(stringbuf + len, buf_len - len,
                   "|cmdq_handle_config=%p\tcmdq_handle_trigger=%p\tdpmgr_"
                   "handle=%p\tovl2mem_path_handle=%p\n",
...
void disp_spm_enter_power_down_mode(void) {
  mmprofile_log_ex(ddp_mmp_get_events()->power_down_mode, MMPROFILE_FLAG_PULSE,
                   0, 0);
}
...
#ifdef MTK_FB_MMDVFS_SUPPORT
  primary_display_request_dvfs_perf(0, HRT_LEVEL_NUM - 1);
#endif
...
#ifdef MTK_FB_MMDVFS_SUPPORT
  primary_display_request_dvfs_perf(0, HRT_LEVEL_LEVEL0);
#endif
...
static int _DC_switch_to_DL_fast(int block) {
  ...
  primary_display_request_dvfs_perf(0, dvfs_last_ovl_req);
#endif
...
static int _build_path_direct_link(void) {
  ...
  pgc->dpmgr_handle =
      dpmgr_create_path(DDP_SCENARIO_PRIMARY_DISP, pgc->cmdq_handle_config);
  if (pgc->dpmgr_handle) {
...
static int _decouple_update_rdma_config_nolock(void) {
  ...
  int layer = 0;
  int ret = 0;
  int composed_size = disp_helper_get_option(DISP_OPT_FAKE_LCM_HEIGHT) *
  ...
  if (primary_display_is_decouple_mode()) {
    ...
    if (dump_output) {
      ...
      if (dump_output_comp) {
        memcpy(composed_buf, decouple_buffer_info[i]->va, composed_size);
    ...
    cmdqRecFlushAsyncCallback(
        cmdq_handle, (CmdqAsyncFlushCB)_Interface_fence_release_callback,
        interface_fence > 1 ? interface_fence - 1 : 0);
...
static int _ovl_fence_release_callback(unsigned long userdata) {
  ...
  if (primary_display_is_video_mode() && !primary_display_is_decouple_mode()) {
    ...
    DISPMSG("ovl config time_event %d time_event1 %d time_event2 %d time1_diff "
            " %d  time2_diff %d\n",
            time_event, time_event1, time_event2, time_event1 - time_event,
...
static int _present_fence_release_worker_thread(void *data) {
  ...
  while (1) {
    ...
    if (!islcmconnected && !primary_display_is_video_mode()) {
      ...
      dpmgr_wait_event_timeout(pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_START,
                               HZ / 10);
    } else {
      dpmgr_wait_event_timeout(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC,
                               HZ / 10);
      mmprofile_log_ex(ddp_mmp_get_events()->present_fence_release,
                       MMPROFILE_FLAG_PULSE, 1, 1);
    }
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps,
  ...
  pm_qos_add_request(&primary_display_emi_opp_request, PM_QOS_DDR_OPP,
                     PM_QOS_DDR_OPP_DEFAULT_VALUE);
  pm_qos_add_request(&primary_display_mm_freq_request, PM_QOS_DISP_FREQ,
  ...
  if (lcm_corner_en)
    primary_display_get_round_corner_mva(
        &top_mva, &bottom_mva, &corner_pattern_width, &corner_pattern_height,
  ...
  if (primary_display_is_video_mode()) {
    ...
    if (_need_lfr_check()) {
      ...
      dpmgr_map_event_to_irq(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC,
                             DDP_IRQ_RDMA0_DONE);
      if (disp_helper_get_option(DISP_OPT_ARR_PHASE_1)) {
        dpmgr_map_event_to_irq(pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_START,
                               DDP_IRQ_RDMA0_START);
      }
    }
  }
...
#ifdef MTK_FB_MMDVFS_SUPPORT
  primary_display_request_dvfs_perf(0, HRT_LEVEL_DEFAULT);
#endif
  return ret;
}
...
int primary_display_resume(void) {
...

  {
    ...
    dpmgr_path_ioctl(pgc->dpmgr_handle, NULL, DDP_OVL_GOLDEN_SETTING,
                     &gset_arg);
  }
  ...
  if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
    mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE,
                     1, 4);
    DISPERR("[POWER]Fatal error, we didn't start display path but it's already "
            "busy\n");
    ret = -1;
    /* goto done; */
  }
  ...
  if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
    mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE,
                     1, 6);
    DISPERR("[POWER]Fatal error, we didn't trigger display path but it's "
            "already busy\n");
    ret = -1;
    /* goto done; */
  }
...
void add_round_corner_layers(struct disp_ddp_path_config *cfg, unsigned int w,
                             unsigned int h, unsigned int h_bot,
                             unsigned int pitch, unsigned long mva_top,
                             unsigned long mva_bot,
...
static bool disp_rsz_frame_has_rsz_layer(struct disp_frame_cfg_t *cfg) {
  ...
  if ((path != 2 && path != 3 && path != 4) && (rsz == true)) {
    ...
    DISPERR("not RPO but L%d(%u,%u,%ux%u)->(%u,%u,%ux%u)\n", i, c->src_offset_x,
            c->src_offset_y, c->src_width, c->src_height, c->tgt_offset_x,
            c->tgt_offset_y, c->tgt_width, c->tgt_height);
  }
...
static void rsz_in_out_roi(struct disp_frame_cfg_t *cfg,
    ...
    if (input_cfg->layer_enable) {
      ...
      if (input_cfg->src_width < input_cfg->tgt_width ||
        ...
        rect_make(&src_layer_roi,
                  (input_cfg->tgt_offset_x * input_cfg->src_width) /
                      input_cfg->tgt_width,
                  (input_cfg->tgt_offset_y * input_cfg->src_height) /
                      input_cfg->tgt_height,
                  input_cfg->src_width, input_cfg->src_height);
        rect_make(&dst_layer_roi, input_cfg->tgt_offset_x,
                  input_cfg->tgt_offset_y, input_cfg->tgt_width,
                  input_cfg->tgt_height);
  ...
  DISPINFO("RPO] rsz_src(x,y,w,h)=(%d,%d,%d,%d)\n", data_config->rsz_src_roi.x,
           data_config->rsz_src_roi.y, data_config->rsz_src_roi.width,
           data_config->rsz_src_roi.height);
  DISPINFO("[RPO] rsz_dst(x,y,w,h)=(%d,%d,%d,%d)\n", data_config->rsz_dst_roi.x,
           data_config->rsz_dst_roi.y, data_config->rsz_dst_roi.width,
...
static void _ovl_sbch_invalid_config(struct cmdqRecStruct *cmdq_handle) {
  ...
  for (j = 0; j < OVL_NUM; j++) {
    ...
    cmdq_op_read_reg(cmdq_handle,
                     disp_addr_convert(DISP_REG_OVL_SBCH_CON + ovl_base),
...
static int _config_ovl_input(struct disp_frame_cfg_t *cfg,
  ...
  if (cfg->setter != SESSION_USER_INVALID) {
    ...
    if (bypass) {
      ...
      if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE) {
        ...
        do_primary_display_switch_mode(DISP_SESSION_RDMA_MODE, pgc->session_id,
                                       0, cmdq_handle, 0);
      }
    } else {
      if (pgc->session_mode == DISP_SESSION_RDMA_MODE) {
  ...
  if (pgc->session_mode != DISP_SESSION_RDMA_MODE) {
    ...
    ret = ddp_convert_ovl_input_to_rdma(
        &data_config->rdma_config, &data_config->ovl_config[bypass_layer_id],
    ...
    dynamic_debug_msg_print(
        data_config->rdma_config.address, data_config->rdma_config.width,
  ...
  if (primary_display_is_decouple_mode() && primary_display_is_mirror_mode()) {
    ...
    primary_display_request_dvfs_perf(0, HRT_LEVEL_LEVEL3);
  ...
  if (primary_display_is_mirror_mode()) {
    if (lcm_corner_en)
      add_round_corner_layers(data_config, primary_display_get_width(),
                              ...
                              DISP_MODULE_OVL0, TOTAL_OVL_LAYER_NUM, 3, 0);
  } else {
    if (lcm_corner_en) {
      if (disp_helper_get_option(DISP_OPT_ROUND_CORNER))
        add_round_corner_layers(
            data_config, primary_display_get_width(), corner_pattern_height,
            corner_pattern_height_bot, corner_pattern_width, top_mva,
            bottom_mva, DISP_MODULE_OVL0, TOTAL_OVL_LAYER_NUM, 3, 1);
      else
        add_round_corner_layers(
            data_config, primary_display_get_width(), corner_pattern_height,
            corner_pattern_height_bot, corner_pattern_width, top_mva,
            bottom_mva, DISP_MODULE_OVL0, TOTAL_OVL_LAYER_NUM, 3, 0);
    }
  }
#endif
  ...
  if (dump_output) {
    ...
    for (i = 0; i < TOTAL_REAL_OVL_LAYER_NUM; i++) {
      ...
      if (ovl_cfg->layer_en == 1) {
        ...
        draw->bot_r_x[l_num] = ovl_cfg->dst_x + ovl_cfg->dst_w - 1;
        draw->bot_r_y[l_num] = ovl_cfg->dst_y + ovl_cfg->dst_h - 1;
        draw->frame_width[l_num] = 6;
        if ((draw->dy[l_num] > primary_display_get_height() - 4 * 16))
          draw->dy[l_num] = primary_display_get_height() - 4 * 16 - 1;

        for (j = 0; j < draw->layer_num - 1; j++) {
          if ((draw->dx[l_num] == draw->dx[j]) &&
              (draw->dy[l_num] == draw->dy[j])) {
            draw->dx[l_num] = draw->dx[j] + 4 * 8;
          }
          if (((ovl_cfg->dst_x <= draw->top_l_x[j] + draw->frame_width[j]) &&
               (ovl_cfg->dst_x >= draw->top_l_x[j])) ||
              ((ovl_cfg->dst_y <= draw->top_l_x[j] + draw->frame_width[j]) &&
               (ovl_cfg->dst_y >= draw->top_l_x[j])) ||
              ((ovl_cfg->dst_x + ovl_cfg->dst_w >=
                draw->bot_r_x[j] - draw->frame_width[j]) &&
               (ovl_cfg->dst_x + ovl_cfg->dst_w <= draw->bot_r_x[j])) ||
              ((ovl_cfg->dst_y + ovl_cfg->dst_h >=
                draw->bot_r_y[j] - draw->frame_width[j]) &&
               (ovl_cfg->dst_y + ovl_cfg->dst_h <= draw->bot_r_y[j])))
            draw->frame_width[l_num] += 3;
          if (draw->top_l_y[l_num] + draw->frame_width[l_num] >
              draw->bot_r_y[l_num])
            draw->frame_width[l_num] =
                draw->bot_r_y[l_num] - draw->top_l_y[l_num];
        }
      }
    }
  }
  ...
  for (i = 0; i < OVL_NUM; i++) {
    if (data_config->read_dum_reg[i]) {
      ...
      cmdqRecBackupRegisterToSlot(
          cmdq_handle, pgc->ovl_dummy_info, i,
          disp_addr_convert(DISP_REG_OVL_DUMMY_REG + ovl_base));
    }
  }
...
int do_primary_display_switch_mode(int sess_mode, unsigned int session,
  ...
  if (pgc->state == DISP_SLEPT) {
    DISPWARN("primary display switch from %s to %s in suspend state!!!\n",
...
int primary_display_force_set_vsync_fps(unsigned int fps,
  ...
  if (!primary_display_is_video_mode()) {
    DISPWARN("currently display is not video mode. fps changing fail\n");
    return -1;
  }
...
static int _screen_cap_by_cmdq(unsigned int mva, enum UNIFIED_COLOR_FMT ufmt,
  ...
  ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_SCREEN_CAPTURE, &cmdq_wait_handle);
  if (ret) {
    DISPCHECK("primary capture:Fail to create primary cmdq wait handle for "
              "capture\n");
    ret = -1;
    goto out;
  }
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf,
  ...
  ion_display_handle = disp_ion_alloc(
      ion_display_client, ION_HEAP_MULTIMEDIA_MAP_MVA_MASK, pbuf, buffer_size);
  ...
  disp_ion_get_mva(ion_display_client, ion_display_handle, &mva, 0,
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf,
  ...
  ret = m4u_cache_sync(m4uClient, DISP_M4U_PORT_DISP_WDMA0, pbuf, buffer_size,
...
int primary_display_capture_framebuffer(unsigned long pbuf) {
  ...
  DISPMSG("w_xres = %d, h_yres = %d, w_fb = %d, pixel_bpp = %d, fbsize = %d, "
          "fbaddress = 0x%08lx\n",
...
int primary_display_resolution_test(void) {
  ...
  for (i = 0; i < sizeof(width_array) / sizeof(int); i++) {
    ...
    if (dpmgr_path_is_busy(pgc->dpmgr_handle))
      DISPERR("[display_test]==>Fatal error, we didn't trigger display path "
...
int primary_display_check_test(void) {
  ...
  dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);
  if (dpmgr_path_is_busy(pgc->dpmgr_handle))
    DISPERR("[display_test]==>Fatal error, we didn't trigger display path but "
            "it's already busy\n");
