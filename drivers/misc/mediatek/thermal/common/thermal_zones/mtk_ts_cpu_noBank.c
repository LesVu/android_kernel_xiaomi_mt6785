...
static int trip_temp[10] = {
    TZCPU_INITCFG_TRIP_0_TEMP, TZCPU_INITCFG_TRIP_1_TEMP,
    TZCPU_INITCFG_TRIP_2_TEMP, TZCPU_INITCFG_TRIP_3_TEMP,
    TZCPU_INITCFG_TRIP_4_TEMP, TZCPU_INITCFG_TRIP_5_TEMP,
    TZCPU_INITCFG_TRIP_6_TEMP, TZCPU_INITCFG_TRIP_7_TEMP,
    TZCPU_INITCFG_TRIP_8_TEMP, TZCPU_INITCFG_TRIP_9_TEMP};
#else
 ...
 *=============================================================
 */
unsigned int __attribute__((weak)) mt_gpufreq_get_max_power(void) {
...
void __attribute__((weak)) mt_cpufreq_thermal_5A_limit(bool enable) {
  pr_notice("E_WF: %s doesn't exist\n", __func__);
}
...
unsigned int __attribute__((weak)) mt_ppm_thermal_get_max_power(void) {
  pr_notice("E_WF: %s doesn't exist\n", __func__);
  return 0;
}

unsigned int __attribute__((weak)) mt_gpufreq_get_seg_max_opp_index(void) {
  pr_notice("E_WF: %s doesn't exist\n", __func__);
  return 0;
}

unsigned int __attribute__((weak)) mt_gpufreq_get_dvfs_table_num(void) {
  pr_notice("E_WF: %s doesn't exist\n", __func__);
  return 0;
}
...
long long int thermal_get_current_time_us(void) {
  ...
  temp = (((long long int)t.tv_sec) * 1000000 + t.tv_usec);
...
int mtk_gpufreq_register(struct mt_gpufreq_power_table_info *freqs, int num) {
  ...
  mtk_gpu_power =
      kzalloc((num) * sizeof(struct mt_gpufreq_power_table_info), GFP_KERNEL);
...
void tscpu_set_GPIO_toggle_for_monitor(void) {
  ...
  tscpu_dprintk("%s,g_GPIO_out_enable=%d\n", __func__, g_GPIO_out_enable);
  ...
  if (g_GPIO_out_enable == 1) {
    if (g_max_temp > g_trigger_temp) {
      ...
      tscpu_printk("%s:lv_GPIO118_MODE=0x%x,", __func__, lv_GPIO118_MODE);
...
static ssize_t tscpu_write_GPIO_out(struct file *file,
  ...
  if (sscanf(desc, "%9s %d %9s %d ", TEMP, &valTEMP, ENABLE, &valENABLE) == 4) {
    ...
    if (!strcmp(TEMP, "TEMP")) {
      ...
      tscpu_printk("%s TEMP bad argument\n", __func__);
    ...
    if (!strcmp(ENABLE, "ENABLE")) {
      ...
      tscpu_printk("%s ENABLE bad argument\n", __func__);
...
#if defined(THERMAL_VPU_SUPPORT)
  seq_printf(m, ",%d", (int)((vpu_power != 0x7FFFFFFF) ? vpu_power : 0));
#endif
#if defined(THERMAL_MDLA_SUPPORT)
  seq_printf(m, ",%d", (int)((mdla_power != 0x7FFFFFFF) ? mdla_power : 0));
#endif
...
static ssize_t tscpu_write_sspm_thermal_throttle(struct file *file,
  ...
  if (kstrtoint(desc, 10, &sspm_thermal_throttle_switch) == 0) {
    tscpu_sspm_thermal_throttle = sspm_thermal_throttle_switch;

    tscpu_warn("%s , %d\n", __func__, tscpu_sspm_thermal_throttle);
...
static ssize_t tscpu_write(struct file *file, const char __user *buffer,
  ...
  if (sscanf(ptr_mtktscpu_data->desc,
    ...
    tscpu_dprintk(
        ...
        __func__, g_THERMAL_TRIP[0], g_THERMAL_TRIP[1], g_THERMAL_TRIP[2]);
    ...
    for (i = 0; i < num_trip; i++) {
      ...
      if (i != 0 && trip_temp[i] > 57000)
        tscpu_printk("%s trip temp %d is over 57'C (%d)!\n", __func__, i,
                     trip_temp[i]);
    }
    ...
    tscpu_dprintk("trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,trip_8_temp=%d,"
                  "trip_9_temp=%d,",
...
static int tscpu_register_thermal(void) {
  ...
  thz_dev = mtk_thermal_zone_device_register(
      "mtktscpu", num_trip, NULL, &mtktscpu_dev_ops, 0, 0, 0, interval);
...
static void tscpu_clear_all_temp(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(lvts_tscpu_g_tc); i++)
    for (j = 0; j < lvts_tscpu_g_tc[i].ts_number; j++)
      tscpu_ts_lvts_temp[lvts_tscpu_g_tc[i].ts[j]] = CLEAR_TEMP;
#endif
}
...
static void read_all_tc_temperature(void) {
  ...
  while (ret == 0 && cunt < 20) {
    ...
    cunt++;
    mdelay(2);
    tscpu_printk("%s, %d,%d,%d\n", __func__, cunt, ret, g_is_temp_valid);
  }
...
static int ktp_thread(void *arg) {
  ...
  for (;;) {
    ...
    if ((temp_tc_mid_trip > -275000) &&
      ...
      tscpu_dprintk("%s else temp=%d, trip=%d, ltd=%d\n", __func__, max_temp,
...
static int lvts_time_profiling_open_opp(struct inode *inode,
                                        struct file *file) {
...
#if defined(THERMAL_SSPM_THERMAL_THROTTLE_SWITCH)
static int tscpu_sspm_thermal_throttle_open(struct inode *inode,
                                            struct file *file) {
...
static int tscpu_read_ttpct(struct seq_file *m, void *v) {
  ...
  cpu_power =
      (cpu_power == 0x7FFFFFFF || cpu_power == 0) ? max_cpu_pwr : cpu_power;

  cpu_power = (cpu_power < max_cpu_pwr) ? cpu_power : max_cpu_pwr;

  gpu_power =
      (gpu_power == 0x7FFFFFFF || gpu_power == 0) ? max_gpu_pwr : gpu_power;
...
int tscpu_get_cpu_temp_met(enum mtk_thermal_sensor_cpu_id_met id) {
  ...
  if (id == ATM_CPU_LIMIT)
    return (adaptive_cpu_power_limit != 0x7FFFFFFF) ? adaptive_cpu_power_limit
                                                    : 0;

  if (id == ATM_GPU_LIMIT)
    return (adaptive_gpu_power_limit != 0x7FFFFFFF) ? adaptive_gpu_power_limit
                                                    : 0;
...
int tscpu_get_temperature_range(void) { return g_is_TempOutsideNormalRange; }
...
static void tscpu_create_fs(void) {
  ...
  if (!mtktscpu_dir) {
    ...
    entry = proc_create("tzcpu", 0664, mtktscpu_dir, &mtktscpu_fops);

    if (entry)
      proc_set_user(entry, uid, gid);

    entry = proc_create("tzcpu_log", 0644, mtktscpu_dir, &mtktscpu_log_fops);

    entry = proc_create("thermlmt", 0444, NULL, &mtktscpu_opp_fops);

#if LVTS_VALID_DATA_TIME_PROFILING
    entry = proc_create("timeProfiling", 0444, mtktscpu_dir,
                        &lvts_time_profiling_opp_fops);
#endif

    entry = proc_create("ttpct", 0444, NULL, &mtktscpu_ttpct_fops);

    entry = proc_create("tzcpu_cal", 0400, mtktscpu_dir, &mtktscpu_cal_fops);

    entry = proc_create("tzcpu_read_temperature", 0444, mtktscpu_dir,
    ...
    entry = proc_create("tzcpu_Tj_out_via_HW_pin", 0644, mtktscpu_dir,
                        &mtktscpu_Tj_out_fops);

    if (entry)
      proc_set_user(entry, uid, gid);
#if THERMAL_GPIO_OUT_TOGGLE
    entry = proc_create("tzcpu_GPIO_out_monitor", 0644, mtktscpu_dir,
                        &mtktscpu_GPIO_out_fops);
    if (entry)
      proc_set_user(entry, uid, gid);
#endif
#if defined(THERMAL_SSPM_THERMAL_THROTTLE_SWITCH)
    entry = proc_create("sspm_thermal_throttle", 0644, mtktscpu_dir,
