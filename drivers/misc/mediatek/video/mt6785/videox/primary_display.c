...
int primary_display_config_full_roi(struct disp_ddp_path_config *pconfig,
  ...
  if (!rect_equal(&total_dirty_roi, &pconfig->ovl_partial_roi)) {
    ...
    if (disp_helper_get_option(DISP_OPT_DYNAMIC_RDMA_GOLDEN_SETTING)) {
      ...
      dpmgr_path_ioctl(disp_handle, cmdq_handle, DDP_RDMA_GOLDEN_SETTING,
                       pconfig);
    }
...
static enum DISP_POWER_STATE
primary_set_state(enum DISP_POWER_STATE new_state) {
...
int dynamic_debug_msg_print(unsigned int mva, int w, int h, int pitch,
  ...
  ret = MFC_Open(&mfc_handle, (void *)kva, pitch, h, Bpp, DAL_COLOR_WHITE,
                 DAL_COLOR_RED);
...
static int primary_show_basic_debug_info(struct disp_frame_cfg_t *cfg) {
  ...
  for (i = 0; i < cfg->input_layer_num; i++) {
    if (cfg->input_cfg[i].tgt_offset_y != 0 || !cfg->input_cfg[i].layer_enable)
      continue;
...
static int fps_ctx_update(struct fps_ctx_t *fps_ctx) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->fps_set, MMPROFILE_FLAG_PULSE, abs_fps,
                   fps_ctx->cur_wnd_sz);
...
static int fps_ctx_get_fps(struct fps_ctx_t *fps_ctx, unsigned int *fps,
  ...
  *stable = 1;
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->fps_get, MMPROFILE_FLAG_PULSE, *fps,
                   *stable);
  mutex_unlock(&fps_ctx->lock);
  return 0;
}
...
static int fps_ext_ctx_update(struct fps_ext_ctx_t *fps_ctx) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->fps_ext_set, MMPROFILE_FLAG_PULSE,
                   fps_ctx->fps, fps_ctx->stable);
  mutex_unlock(&fps_ctx->lock);
...
static int fps_ext_ctx_get_fps(struct fps_ext_ctx_t *fps_ctx, unsigned int *fps,
  ...
  *stable = fps_ctx->stable;
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->fps_ext_get, MMPROFILE_FLAG_PULSE,
                   fps_ctx->fps, fps_ctx->stable);
  mutex_unlock(&fps_ctx->lock);
...
int lcm_fps_ctx_update(struct lcm_fps_ctx_t *fps_ctx,
  ...
  if (fps_ctx->num < LCM_FPS_ARRAY_SIZE)
    ...
    fps_ctx->head_idx = (fps_ctx->head_idx + 1) % LCM_FPS_ARRAY_SIZE;
...
unsigned int lcm_fps_ctx_get(struct lcm_fps_ctx_t *fps_ctx) {
  ...
  if (fps_ctx->num <= 3) {
    DISPMSG("%s num is %d which is < 3, so return fix fps", __func__,
            fps_ctx->num);
...
int primary_display_get_debug_state(char *stringbuf, int buf_len) {
  ...
  if (lcm_param && lcm_drv)
    len += scnprintf(stringbuf + len, buf_len - len,
                     "|LCM Driver=[%s], Resolution=%dx%d, Interface:%s, LCM "
                     "Connected:%s, color_mode=%s\n",
  ...
  len += scnprintf(
      ...
      pgc->max_layer, session_mode_str(pgc->session_mode), pgc->vsync_drop);
  len += scnprintf(stringbuf + len, buf_len - len,
  ...
  for (i = 0; i < HRT_LEVEL_NUM - 1; i++)
    len += scnprintf(stringbuf + len, buf_len - len, "%d, ", hrt_table[i]);
  len += scnprintf(stringbuf + len, buf_len - len, "%d]\n",
                   hrt_table[HRT_LEVEL_NUM - 1]);

  /*ARR info*/
  len += scnprintf(stringbuf + len, buf_len - len, "|arr=%d\n",
                   primary_display_is_support_ARR());

#ifdef CONFIG_MTK_HIGH_FRAME_RATE
  /*DynFPS info*/
  len += scnprintf(stringbuf + len, buf_len - len, "|DynFPS=%d\n",
...
static void _cmdq_build_trigger_loop(void) {
  ...
  if (primary_display_is_video_mode()) {
    ...
    if (primary_display_is_support_ARR()) {
       ...
       */
      cmdq_op_read_reg(pgc->cmdq_handle_trigger, pgc->trigger_record_slot,
                       &trigger_run_time_var, ~0);
      cmdq_op_if(pgc->cmdq_handle_trigger, trigger_run_time_var, CMDQ_EQUAL, 0);
      cmdq_op_add(pgc->cmdq_handle_trigger, &trigger_run_time_var,
                  trigger_run_time_var, 1);
      cmdq_op_write_reg(pgc->cmdq_handle_trigger, pgc->trigger_record_slot,
                        trigger_run_time_var, ~0);
    ...
    if (primary_display_is_support_ARR()) {
      ...
      if ((_get_dst_module_by_lcm(pgc->plcm) == DISP_MODULE_DSIDUAL) &&
          dsi_vfp_addr[1]) {
        cmdqRecBackupWriteRegisterFromSlot(pgc->cmdq_handle_trigger,
                                           pgc->dsi_vfp_line, 0, addr);
      }
    ...
    if (need_wait_esd_eof()) {
      ...
      ret =
          cmdqRecWaitNoClear(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_ESD_EOF);
    }
    /*
...
#endif
    ret =
        cmdqRecWaitNoClear(pgc->cmdq_handle_trigger, CMDQ_SYNC_TOKEN_CABC_EOF);
    /*
...
#ifdef MTK_FB_MMDVFS_SUPPORT
  prim_disp_request_hrt_bw(dvfs_last_ovl_req, DDP_SCENARIO_PRIMARY_ALL,
                           __func__, active_cfg);
#endif
...
static int sec_buf_ion_alloc(int buf_size) {
  ...
  sec_ion_handle =
      ion_alloc(ion_client, buf_size, align, ION_HEAP_MULTIMEDIA_SEC_MASK, 0);
...
static int _DL_switch_to_DC_fast(int block) {
...
#ifdef MTK_FB_MMDVFS_SUPPORT
  prim_disp_request_hrt_bw(2, DDP_SCENARIO_PRIMARY_RDMA0_COLOR0_DISP, __func__,
                           active_cfg);
#endif
  ...
  memset(&io_gs, 0, sizeof(io_gs));
  io_gs.dst_mod_type = dpmgr_path_get_dst_module_type(pgc->ovl2mem_path_handle);
  io_gs.is_decouple_mode = 1;
...
static int _DC_switch_to_DL_fast(int block) {
  ...
  switch_module_to_nonsec(pgc->ovl2mem_path_handle, NULL, DISP_MODULE_WDMA0,
                          __func__);
#endif
  ...
  prim_disp_request_hrt_bw(dvfs_last_ovl_req, DDP_SCENARIO_PRIMARY_DISP,
                           __func__, active_cfg);
#endif
...
static int DL_switch_to_rdma_mode(struct cmdqRecStruct *handle, int block) {
  ...
  dpmgr_modify_path_power_on_new_modules(pgc->dpmgr_handle, new_scn, 0);
  dpmgr_modify_path(
      pgc->dpmgr_handle, new_scn, handle,
      primary_display_is_video_mode() ? DDP_VIDEO_MODE : DDP_CMD_MODE, 0);
  dpmgr_modify_path_power_off_old_modules(old_scn, new_scn, 0);
  ...
  io_gs.dst_mod_type = dpmgr_path_get_dst_module_type(pgc->dpmgr_handle);
  io_gs.is_decouple_mode = 1;
  dpmgr_path_ioctl(pgc->dpmgr_handle, handle, DDP_OVL_GOLDEN_SETTING, &io_gs);
...
#endif
    pgc->dc_buf[i] =
        i * buffer_size + primary_display_get_frame_buffer_mva_address();
  }
...
int _trigger_ovl_to_memory(disp_path_handle disp_handle,
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->ovl_trigger, MMPROFILE_FLAG_START, 0,
                   data);
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->ovl_trigger, MMPROFILE_FLAG_END, 0,
                   data);
...
static void DC_config_nightlight(struct cmdqRecStruct *cmdq_handle) {
  ...
  for (i = 0; i < 16; i++)
    cmdqBackupReadSlot(pgc->night_light_params, i + 1, &(ccorr_matrix[i]));
...
static int _decouple_update_rdma_config_nolock(void) {
  ...
  if (primary_get_state() != DISP_ALIVE) {
    ...
    _Interface_fence_release_callback(interface_fence > 1 ? interface_fence - 1
                                                          : 0);
...
#ifdef _DEBUG_DITHER_HANG_
  if (primary_display_is_video_mode()) {
    cmdqRecBackupRegisterToSlot(cmdq_handle, pgc->dither_status_info, 0,
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_rdma_config,
                   MMPROFILE_FLAG_PULSE, interface_fence, tmpConfig.address);
...
static int _ovl_fence_release_callback(unsigned long userdata) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->session_release, MMPROFILE_FLAG_START,
                   1, userdata);
  ...
  if (primary_display_is_support_ARR()) {
    ...
    if (vfp_changed) {
      ...
      cmdqBackupReadSlot(pgc->next_working_fps, 0, &pgc->working_dfps);
      pgc->hw_current_fps = pgc->working_dfps;

      /*update idle timeout*/
      new_idle_inteval = primary_display_get_idle_interval(pgc->working_dfps);
      old_idle_inteval = disp_lp_set_idle_check_interval(new_idle_inteval);
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->session_release, MMPROFILE_FLAG_END, 1,
                   userdata);
  return ret;
}
...
static int _present_fence_release_worker_thread(void *data) {
  ...
  while (1) {
    ...
    cmdqBackupReadSlot(pgc->cur_config_fence,
                       disp_sync_get_present_timeline_id(), &pf_idx);
...
unsigned long get_dim_layer_mva_addr(void) {
  ...
  if (dim_layer_mva == 0) {
    ...
    dim_layer_mva =
        pgc->framebuffer_mva + (DISP_GetPages() - 1) * frame_buffer_size;
...
#endif
#ifdef CONFIG_MTK_ROUND_CORNER_SUPPORT
  if (disp_helper_get_option(DISP_OPT_ROUND_CORNER_MODE) == DISP_HELPER_HW_RC)
    DISP_REG_SET_FIELD(pgc->cmdq_handle_config, REG_FLD(1, 0),
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps,
  ...
  unsigned int rc_mode = disp_helper_get_option(DISP_OPT_ROUND_CORNER_MODE);
#endif
  ...
  if (rc_mode == DISP_HELPER_SW_RC) {
    if (lcm_corner_en) {
      get_sw_round_corner_param(&top_mva, &bottom_mva, NULL, NULL,
    ...
    if (lcm_corner_en && !top_mva) {
      ...
      ion_handle = disp_ion_alloc(ion_client, ION_HEAP_MULTIMEDIA_MASK, 0,
                                  lcm_param->corner_pattern_tp_size);
      ...
      if (IS_ERR(rc_va_addr))
        ...
        memcpy(rc_va_addr, lcm_param->corner_pattern_lt_addr,
               lcm_param->corner_pattern_tp_size);

      ion_unmap_kernel(ion_client, ion_handle);

      disp_ion_get_mva(ion_client, ion_handle, &top_mva, 0,
                       DISP_M4U_PORT_DISP_POSTMASK);
      disp_ion_cache_flush(ion_client, ion_handle, ION_CACHE_INVALID_BY_RANGE);
  ...
  init_decouple_buffer_thread =
      kthread_run(_init_decouple_buffers_thread, NULL, "init_decouple_buffer");
  ...
  if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
    primary_delay_trigger_task =
        kthread_create(_disp_primary_path_check_trigger_delay_33ms, NULL,
                       "disp_delay_trigger");
  ...
  if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
    primary_od_trigger_task = kthread_create(
        _disp_primary_path_check_trigger_od, NULL, "disp_od_trigger");
  ...
  if (disp_helper_get_option(DISP_OPT_PRESENT_FENCE)) {
    ...
    present_fence_release_worker_task = kthread_create(
        _present_fence_release_worker_thread, NULL, "present_fence_worker");
  ...
  if (disp_helper_get_option(DISP_OPT_PERFORMANCE_DEBUG)) {
    if (!primary_display_frame_update_task) {
      ...
      disp_register_module_irq_callback(
          DISP_MODULE_OVL0, primary_display_frame_update_irq_callback);

      primary_display_frame_update_task = kthread_create(
          primary_display_frame_update_kthread, NULL, "frame_update_worker");
...
int _display_set_lcm_refresh_rate(int fps) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_switch_fps, MMPROFILE_FLAG_END,
                   fps, 0);
#endif
  return 0;
}
...
int primary_display_release_fence_fake(void) {
  ...
  for (i = 0; i < PRIMARY_SESSION_INPUT_LAYER_COUNT; i++) {
    ...
    disp_sync_get_cached_layer_info(session, i, &layer_en,
...
int primary_display_suspend(void) {
  ...
  DISPCHECK("%s begin\n", __func__);
  mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_START,
                   0, 0);
  primary_display_idlemgr_kick(__func__, 1);
  ...
  _blocking_flush();
  mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_PULSE,
                   0, 1);
  if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
    ...
    DISPCHECK("[POWER]primary display path is busy now, wait frame done, "
              "event_ret=%d\n",
              event_ret);
    if (event_ret <= 0) {
   ...
   */
  prim_disp_request_hrt_bw(HRT_BW_UNREQ, DDP_SCENARIO_PRIMARY_DISP, __func__,
                           active_cfg);
#endif
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_suspend, MMPROFILE_FLAG_END, 0,
                   0);
  DISPCHECK("%s end\n", __func__);
  ddp_clk_check();
...
void primary_display_vdo_restart(bool need_wait_frame_done) {
  ...
  /* stop dsi vdo mode */
  dpmgr_path_build_cmdq(primary_get_dpmgr_handle(), qhandle, CMDQ_STOP_VDO_MODE,
                        0);
...
int primary_display_resume(void) {
  ...
  DISP_PR_INFO("[%s][%d]\n", __func__, __LINE__);
  {
    ...
    for (i = 0; i < ARRAY_SIZE(data_config->ovl_config); i++) {
      if (is_DAL_Enabled() && data_config->ovl_config[i].layer ==
    ...
    dpmgr_path_ioctl(pgc->dpmgr_handle, NULL, DDP_OVL_GOLDEN_SETTING, &io_gs);
  }
  DISP_PR_INFO("[DENNIS][%s][%d]\n", __func__, __LINE__);
  ...
  if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
    mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE,
                     1, 4);
    DISP_PR_ERR("[POWER]Fatal error, we didn't start display path but it's "
                "already busy\n");
    ret = -1;
    /* goto done; */
  }
  ...
  if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
    mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE,
                     1, 6);
    DISP_PR_ERR("[POWER]Fatal error, we didn't trigger display path but it's "
                "already busy\n");
    ret = -1;
    /* goto done; */
  }
...
#endif
  mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_END, 0,
                   0);
  ddp_clk_check();
...
static int trigger_decouple_mirror(void) {
  if (pgc->need_trigger_dcMirror_out == 0) {
    ...
    if (pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE) {
      ...
      dprec_logger_trigger(DPREC_LOGGER_PRIMARY_TRIGGER, 0xffffffff,
                           0xaaaaaaaa);
    }
  }
  return 0;
...
static int setup_disp_sec(struct disp_ddp_path_config *data_config,
  ...
  if (has_sec_layer != primary_is_sec()) {
    mmprofile_log_ex(ddp_mmp_get_events()->sec, MMPROFILE_FLAG_PULSE,
                     has_sec_layer, 0);
    /* sec/nonsec switch */
...
#endif
bool disp_input_has_yuv(void) { return has_yuv_input; }
...
bool disp_idle_check_rsz(void) { return has_rsz_input; }
...
static bool disp_rsz_frame_has_rsz_layer(struct disp_frame_cfg_t *cfg) {
  ...
  for (i = 0; i < cfg->input_layer_num; i++) {
    ...
    if (c->src_width != c->tgt_width || c->src_height != c->tgt_height) {
      rsz = true;
      break;
    }
  }
  ...
  if ((path != 2 && path != 3) && (rsz == true)) {
    ...
    DISP_PR_ERR("not RPO but L%d(%u,%u,%ux%u)->(%u,%u,%ux%u)\n", i,
                c->src_offset_x, c->src_offset_y, c->src_width, c->src_height,
                c->tgt_offset_x, c->tgt_offset_y, c->tgt_width, c->tgt_height);
  }
...
static void disp_rsz_in_out_roi(struct disp_frame_cfg_t *cfg,
  ...
  for (i = 0; i < cfg->input_layer_num; i++) {
    ...
    rect_make(&src_layer_roi, c->tgt_offset_x * c->src_width / c->tgt_width,
              c->tgt_offset_y * c->src_height / c->tgt_height, c->src_width,
              c->src_height);
    rect_make(&dst_layer_roi, c->tgt_offset_x, c->tgt_offset_y, c->tgt_width,
              c->tgt_height);
  ...
  DISPINFO("[RPO] rsz(%d,%d,%dx%d)->(%d,%d,%dx%d)\n", pconfig->rsz_src_roi.x,
           pconfig->rsz_src_roi.y, pconfig->rsz_src_roi.width,
           pconfig->rsz_src_roi.height, pconfig->rsz_dst_roi.x,
           pconfig->rsz_dst_roi.y, pconfig->rsz_dst_roi.width,
           pconfig->rsz_dst_roi.height);
}
...
static void _ovl_sbch_invalid_config(struct cmdqRecStruct *cmdq_handle) {
  ...
  for (j = 0; j < OVL_NUM; j++) {
    ...
    cmdq_op_read_reg(cmdq_handle,
                     disp_addr_convert(DISP_REG_OVL_SBCH_STS + ovl_base),
    ...
    for (i = 0; i < OVL_MODULE_MAX_PHY_LAYER; i++) {
      cmdq_op_assign(cmdq_handle, &shift, (1 << (3 + 4 * i)));
      ...
      cmdq_op_write_reg(cmdq_handle,
                        disp_addr_convert(DISP_REG_OVL_SBCH + ovl_base), 0,
                        (1 << (16 + (i * 4))));
    ...
    for (i = 0; i < OVL_MODULE_MAX_EXT_LAYER; i++) {
      cmdq_op_assign(cmdq_handle, &shift, (1 << (19 + 4 * i)));
      ...
      cmdq_op_write_reg(cmdq_handle,
                        disp_addr_convert(DISP_REG_OVL_SBCH_EXT + ovl_base), 0,
                        (1 << (16 + (i * 4))));
...
static int _config_ovl_input(struct disp_frame_cfg_t *cfg,
  ...
  if (disp_partial_is_support()) {
    if (primary_display_is_directlink_mode() && !dc_request)
      disp_partial_compute_ovl_roi(cfg, pconfig, &total_dirty_roi);
    else
      assign_full_lcm_roi(&total_dirty_roi);
  }
  ...
  if (cfg->setter != SESSION_USER_INVALID) {
    ...
    if (n_mode != o_mode) {
      do_primary_display_switch_mode(n_mode, pgc->session_id, 0, cmdq_handle,
                                     0);
      primary_get_path_handles(&disp_handle, &cmdq_handle);
      pconfig = dpmgr_path_get_last_config(disp_handle);
      assign_full_lcm_roi(&total_dirty_roi);
      if (n_mode != DISP_SESSION_DIRECT_LINK_MODE)
        primary_display_config_full_roi(pconfig, disp_handle, cmdq_handle);
    }
  }
  ...
  if (pgc->session_mode != DISP_SESSION_RDMA_MODE) {
    ...
    dynamic_debug_msg_print(
        pconfig->rdma_config.address, pconfig->rdma_config.width,
        pconfig->rdma_config.height, pconfig->rdma_config.pitch,
        UFMT_GET_Bpp(pconfig->rdma_config.inputFormat));
  }
...
#ifdef MTK_FB_MMDVFS_SUPPORT
  if (primary_display_is_decouple_mode())
    prim_disp_request_hrt_bw(2, DDP_SCENARIO_PRIMARY_RDMA0_COLOR0_DISP,
                             __func__, cfg->active_config);
  else {
    if ((overlap_num - dvfs_last_ovl_req) > 0)
      prim_disp_request_hrt_bw(overlap_num, DDP_SCENARIO_PRIMARY_DISP, __func__,
                               cfg->active_config);
  ...
  if (disp_partial_is_support() && primary_display_is_directlink_mode()) {
    ...
    if ((!aal_is_partial_support() || PanelMaster_is_enable() == 1) &&
        !ddp_debug_force_roi())
      assign_full_lcm_roi(&total_dirty_roi);

    DISPDBG("frame partial roi(%d,%d,%d,%d)\n", total_dirty_roi.x,
            total_dirty_roi.y, total_dirty_roi.width, total_dirty_roi.height);
    ...
    if (ddp_debug_partial_statistic()) {
      ...
      evaluate_bandwidth_save(pconfig, &frame_ori, &frame_partial);
      total_ori += frame_ori;
  ...
  for (i = 0; i < cfg->input_layer_num; i++) {
    ...
    if (cur_fence != -1 && cur_fence > last_fence)
      cmdqRecBackupUpdateSlot(cmdq_handle, pgc->cur_config_fence, l_id,
                              cur_fence);
    ...
    if (l_id == 0)
      sub |= hrt_level << 16;
    cmdqRecBackupUpdateSlot(cmdq_handle, pgc->subtractor_when_free, l_id, sub);
  }

  /* SBCH invalid status judge and handle */
  if (disp_helper_get_option(DISP_OPT_OVL_SBCH) && (pconfig->sbch_enable == 1))
  ...
  for (i = 0; i < OVL_NUM; i++) {
    if (pconfig->read_dum_reg[i]) {
      ...
      cmdqRecBackupRegisterToSlot(
          cmdq_handle, pgc->ovl_sbch_info, i,
          disp_addr_convert(DISP_REG_OVL_SBCH_STS + ovl_base));
    }
  }
...
static int primary_frame_cfg_input(struct disp_frame_cfg_t *cfg) {
  ...
  cmdqRecBackupUpdateSlot(cmdq_handle, pgc->hrt_idx_id, 0, cfg->hrt_idx);

  _config_ovl_input(cfg, disp_handle, cmdq_handle);
#ifdef CONFIG_MTK_HIGH_FRAME_RATE
  pconfig = dpmgr_path_get_last_config(pgc->dpmgr_handle);
  if (primary_display_is_decouple_mode())
    ovl2mem_pconfig = dpmgr_path_get_last_config(pgc->ovl2mem_path_handle);
  ...
  if (last_cfg_id != cfg->active_config) {
    ...
    primary_display_get_cfg_fps(cfg->active_config, NULL, &_timing_fps);
    (pconfig->p_golden_setting_context)->fps = _timing_fps / 100;
    ...
    if (primary_display_is_decouple_mode()) {
      ...
      (ovl2mem_pconfig->p_golden_setting_context)->fps = _timing_fps / 100;
    }
  ...
  cmdqRecBackupUpdateSlot(cmdq_handle, pgc->config_id_slot, 0,
                          cfg->active_config);
#endif
  ...
  if (m_ccorr_config.is_dirty) {
    ...
    if (all_zero)
      ...
      disp_ccorr_set_color_matrix(cmdq_handle, m_ccorr_config.color_matrix,
...
int do_primary_display_switch_mode(int sess_mode, unsigned int session,
  ...
  DISPINFO("%s: require sess_mode %s(from:%s)\n", __func__,
           session_mode_str(sess_mode), session_mode_str(pgc->session_mode));
  ...
  DISPMSG("primary display will switch from %s to %s\n",
          session_mode_str(pgc->session_mode), session_mode_str(sess_mode));
...
int primary_display_force_set_vsync_fps(unsigned int fps,
  ...
  if (scenario == 2) {
    if (primary_display_is_support_ARR() && (arr_fps_enable == 1)) {
      /*
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_req,
                   MMPROFILE_FLAG_END, dynamic_fps_changed, pgc->dynamic_fps);
done:
  _primary_path_unlock(__func__);
  return ret;
}
...
int primary_display_setbacklight_nolock(unsigned int level) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl, MMPROFILE_FLAG_END, 0,
                   0);
  return 0;
}
...
int _set_lcm_cmd_by_cmdq(unsigned int *lcm_cmd, unsigned int *lcm_count,
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_set_cmd, MMPROFILE_FLAG_PULSE,
                   1, 1);
  ...
  if (primary_display_is_video_mode()) {
    ...
    disp_lcm_set_lcm_cmd(pgc->plcm, cmdq_handle_lcm_cmd, lcm_cmd, lcm_count,
                         lcm_value);
    dpmgr_path_build_cmdq(primary_get_dpmgr_handle(), cmdq_handle_lcm_cmd,
                          CMDQ_START_VDO_MODE, 0);
    cmdqRecClearEventToken(cmdq_handle_lcm_cmd, CMDQ_EVENT_MUTEX0_STREAM_EOF);

    dpmgr_path_trigger(primary_get_dpmgr_handle(), cmdq_handle_lcm_cmd,
                       CMDQ_ENABLE);
    ddp_mutex_set_sof_wait(dpmgr_path_get_mutex(primary_get_dpmgr_handle()),
                           cmdq_handle_lcm_cmd, 0);
...
int primary_display_setlcm_cmd(unsigned int *lcm_cmd, unsigned int *lcm_count,
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_set_cmd, MMPROFILE_FLAG_START,
                   0, 0);
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_set_cmd, MMPROFILE_FLAG_END, 0,
                   0);
...
int primary_display_mipi_clk_change(unsigned int clk_value) {
  ...
  dpmgr_path_ioctl(primary_get_dpmgr_handle(), cmdq_handle, DDP_PHY_CLK_CHANGE,
                   &clk_value);
...
int primary_display_capture_framebuffer(unsigned long pbuf) {
  ...
  DISPMSG("w_res=%d, h_yres=%d, pixel_bpp=%d, w_fb=%d, fbsize=%d, "
          "fbaddress=0x%lx\n",
          w_xres, h_yres, Bpp, w_fb, fbsize, fbaddress);

  fbv = ioremap(fbaddress, fbsize);
  DISPMSG("w_xres=%d, h_yres=%d, w_fb=%d, pixel_bpp=%d, fbsize=%d, fbaddress = "
          "0x%08lx\n",
          w_xres, h_yres, w_fb, Bpp, fbsize, fbaddress);
  if (!fbv) {
    DISPMSG("[FB Driver], Unable to allocate memory for frame buffer: "
            "address=0x%lx, size=0x%08x\n",
            fbaddress, fbsize);
    return -1;
  }
  ...
  for (i = 0; i < h_yres; i++) {
    memcpy((void *)(pbuf + i * w_xres * Bpp), (void *)(fbv + i * w_fb * Bpp),
           w_xres * Bpp);
  }
...
UINT32 DISP_GetFBRamSize(void) {
  ...
  if (!fb_size_cal) {
    ...
    body_buf_size = ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) *
                    ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT) *
                    ((DISP_GetScreenBpp() + 7) >> 3);

    header_buf_size = (ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) *
                       ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT)) /
    ...
    afbc_frame_buf_size = (body_buf_size + header_buf_size) * DISP_GetPages();
...
int primary_display_lcm_ATA(void) {
  ...
  /*it is easy to cause screen flicker when stop video mode and read ddic by
   * cpu*/
...
int fbconfig_get_esd_check_test(UINT32 dsi_id, UINT32 cmd, UINT8 *buffer,
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->esd_check_t, MMPROFILE_FLAG_START, cmd,
                   -1);
...
int primary_display_resolution_test(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(width_array); i++) {
    ...
    if (dpmgr_path_is_busy(pgc->dpmgr_handle))
      DISP_PR_ERR("[display_test]==>Fatal error, we didn't trigger display "
                  "path but it's already busy\n");
...
int primary_display_check_test(void) {
  ...
  dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);
  if (dpmgr_path_is_busy(pgc->dpmgr_handle))
    DISP_PR_ERR("[display_test]==>Fatal error, we didn't trigger display path "
...
int display_enter_tui(void) {
  ...
  if (disp_helper_get_option(DISP_OPT_TUI_MODE) == TUI_SINGLE_WINDOW_MODE) {
    do_primary_display_switch_mode(DISP_SESSION_DECOUPLE_MODE, pgc->session_id,
                                   0, NULL, 0);
    ...
    DISPMSG("[cc]%s:set module driver(OVL0_2L):%p\n", __func__,
            ddp_get_module_driver(DISP_MODULE_OVL0_2L));
  } else {
...
int display_exit_tui(void) {
  ...
  /* msleep(32); */
  do_primary_display_switch_mode(tui_session_mode_backup, pgc->session_id, 0,
                                 NULL, 0);
...
static int primary_display_enter_self_refresh(void) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->self_refresh, MMPROFILE_FLAG_START, 0,
                   0);
  ...
  do_primary_display_switch_mode(DISP_SESSION_DIRECT_LINK_MODE, pgc->session_id,
                                 0, NULL, 1);
...
out:
  _primary_path_unlock(__func__);
  mmprofile_log_ex(ddp_mmp_get_events()->self_refresh, MMPROFILE_FLAG_END, 0,
                   0);
  return 0;
}
...
int primary_display_set_scenario(int scenario) {
  ...
  if (scenario != DISP_SCENARIO_NORMAL &&
    ...
    pr_err("%s set scenario %d fail ! current scenario is %d\n", __func__,
           scenario, pgc->primary_display_scenario);
    return -EINVAL;
  }
...
int primary_display_wait_fps_change(unsigned int *new_fps) {
  ...
  mmprofile_log_ex(
      ddp_mmp_get_events()->primary_chg_fps_wait, MMPROFILE_FLAG_START,
      atomic_read(&primary_display_fps_chg_trigger), pgc->working_dfps);
  ...
  *new_fps = pgc->working_dfps;
  ...
  mmprofile_log_ex(
      ddp_mmp_get_events()->primary_chg_fps_wait, MMPROFILE_FLAG_END,
      atomic_read(&primary_display_fps_chg_trigger), pgc->working_dfps);
  return ret;
}
...
int primary_display_get_dynamic_fps_info(
  ...
  tmp_fps_levl =
      tmp_fps_levl < DYNAMIC_FPS_LEVELS ? tmp_fps_levl : DYNAMIC_FPS_LEVELS;
  DISPMSG("%s,fps level num :%d\n", __func__, tmp_fps_levl);

  fps_levels->fps_level_num = tmp_fps_levl;
  for (i = 0; i < tmp_fps_levl; i++) {
    (fps_levels->fps_levels)[i] = (params->dsi).dynamic_fps_table[i].fps;
...
unsigned int primary_display_is_support_ARR(void) {
  ...
  if (disp_helper_get_option(DISP_OPT_ARR_PHASE_1) &&
      primary_display_is_video_mode() && disp_lcm_is_arr_support(pgc->plcm)) {
...
unsigned int primary_display_get_dyn_fps(unsigned int VFP_PORTCH) {
  ...
  /*look up table for vfp related to fps*/
  for (i = 0; i < fps_levels; i++) {
    if (VFP_PORTCH == ((params->dsi).dynamic_fps_table[i]).vfp) {
...
void primary_display_notify_fps_chg(unsigned int notify_fps) {
  ...
  mmprofile_log_ex(ddp_mmp_get_events()->primary_chg_fps_notify,
                   ...
                   atomic_read(&primary_display_fps_chg_trigger), notify_fps);
...
void primary_display_update_vfp_line_slot(struct cmdqRecStruct *handle,
                                          unsigned int apply_vfp) {
...
int primary_display_get_cfg_fps(int config_id, unsigned int *fps,
...
void primary_display_dynfps_chg_fps(int cfg_id) {
  ...
  if (fps_change_index & DYNFPS_DSI_MIPI_CLK ||
    ...
    if (need_send_cmd || (fps_change_index & DYNFPS_DSI_MIPI_CLK)) {
      ...
      dpmgr_path_build_cmdq(pgc->dpmgr_handle, qhandle, CMDQ_STOP_VDO_MODE, 0);
    }
    if (need_send_cmd) {
      DISPMSG("%s,send cmd to lcm\n", __func__);
      disp_lcm_dynfps_send_cmd(pgc->plcm, qhandle, last_dynfps, new_dynfps);
    }

    ddp_dsi_dynfps_chg_fps(DISP_MODULE_DSI0, qhandle, last_dynfps, new_dynfps,
                           fps_change_index);
    ...
    ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK, &qhandle);
    ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK, &qhandle2);
...
void primary_display_dynfps_get_vfp_info(unsigned int *vfp,
                                         unsigned int *vfp_for_lp) {
  unsigned int cfg_id;
  unsigned int fps;
...
int primary_display_set_panel_param(unsigned int param) {
  ...
  if (pgc->state == DISP_SLEPT) {
    ...
    if (primary_display_cmdq_enabled()) {
      if (primary_display_is_video_mode()) {
        mmprofile_log_ex(ddp_mmp_get_events()->dsi_wrlcm, MMPROFILE_FLAG_PULSE,
                         0, 7);
